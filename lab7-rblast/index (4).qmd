...
billboarddf$sumwk1wk2 =  billboarddf$wk1 + billboarddf$wk2
```
...
```{r}
long_billboard_sorted <- billboard3 %>% arrange(date, rank, artist, track)
...
flights_weather_right_joined <- left_join(weather, flights, by=c("year", "month", "day", "hour", "origin"))
...
average_delay_per_wind_condition <- flights_weather_inner_joined %>%
  group_by(wind_dir) %>%
  summarise(Average_Departure_Delay = mean(dep_delay, na.rm = TRUE))

average_delay_per_wind_condition
```
...
```{r}
library(nycflights13)
long_delays<-flights[flights$dep_delay>720|flights$arr_delay>720,]
counts<-table(long_delays$origin)
...
```{r}
library(nycflights13)
jfk_to_lax<-flights[flights$origin=="JFK"&flights$dest=="LAX",]
avg_air_time_hours<-mean(jfk_to_lax$air_time,na.rm=TRUE)/60
num_flights<-nrow(jfk_to_lax)
result<-data.frame(
  avg_air_time_hours=avg_air_time_hours,
...
library(nycflights13)
str(airports)
tz_counts<-table(airports$tz)
tz_counts<-sort(tz_counts,decreasing=TRUE)
tz_counts
missing_tz_airports<-airports[is.na(airports$tz),]
missing_tz_airports[1,c("name","faa","tz")]
```